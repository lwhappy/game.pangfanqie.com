...
ig.module('game.entities.enemy').requires('impact.entity', 'impact.font', 'game.avenir-next', 'game.entities.explosion').defines(function() {
    EntityEnemy = ig.Entity.extend({
        ...
        init: function(x, y, settings) {
            ...
            ig.game.initCurrentTarget()//by lw  初始化一个目标
            this.emps = 3
            /*if(ig.game.wave.wave<=3){
                this.emps -= ig.game.wave.wave
            }else{
                this.emps = 0
            }*/
        ...
        getWordWithLength: function(l) {
            ...
            for (var i = 0; i < 20; i++) {
                ...
                // }
                // by lw
                if(typeof ig.game.wordStart === 'number' && ig.game.wordStart < ig.game.wordlist.length){
                   ig.game.wordStart ++ 
                }else{
                    ig.game.wordStart = 0
                }
                w = ig.game.wordlist[ig.game.wordStart];
        ...
        drawLabel: function() {
            ...
            if (this.targeted) {
                let remainingWord = this.remainingWord
                if(!this.isShowWord){
                    remainingWord = remainingWord.split('')
                    for(let i = 0; i < remainingWord.length; i++){
                            remainingWord[i] = '*'
                    }
                    remainingWord = remainingWord.join('')
                }
                let word = this.hitWord + remainingWord
                ig.system.context.fillText(chineseOfWord[this.word],bx,by + 48)
                this.fontActive.draw(word, x + 2, y + 6, ig.Font.ALIGN.RIGHT);
            } 
            else {
        ...
        isHitBy: function(letter) {
            ...
            if (expected == letter.toLowerCase()) {
                this.hitWord = this.hitWord += this.remainingWord.charAt(0)
...
ig.module('game.entities.player').requires('impact.entity', 'impact.entity-pool', 'game.entities.emp', 'game.entities.particle').defines(function() {
    EntityPlayer = ig.Entity.extend({
        ...
        showWord: function() {
            if (ig.game.currentTarget.emps) {
                ig.game.currentTarget.isShowWord = true
                setTimeout(()=>{
                    ig.game.currentTarget.isShowWord = false
                },3000)
                ig.game.currentTarget.emps--;
            }
        },
        kill: function() {
            ig.game.setGameOver();
...
ig.module('game.keyboard').requires('impact.font').defines(function() {
    ig.Keyboard = ig.Class.extend({
        ...
        drawFull: function() {
            ...
            if(ig.game.currentTarget){
                this.empButton.drawTile(this.x + 582, this.y + 250, ig.game.currentTarget.emps, 34, 40);
            }
...
ig.module('game.main').requires('impact.game', 'impact.font', 'game.menus.about', 'game.menus.game-over', 'game.menus.pause', 'game.menus.title', 'game.entities.enemy-missle', 'game.entities.enemy-mine', 'game.entities.enemy-destroyer', 'game.entities.enemy-oppressor', 'game.entities.player', 'game.keyboard', 'game.xhr', 'game.ease', 'plugins.silent-loader', 'plugins.rise-loader', 'game.document-scanner', 'game.words.en').defines(function() {
    ...
    ZType = ig.Game.extend({
        ...
        nextWave: function() {
            // by lw
            var wordIndex = (ig.game.wave.wave) % (ig.WORDS.EN.length -1)
            this.wave.wave++;
            this.wordlist = ig.WORDS.EN[wordIndex];
            var num = getQueryVariable("num")
            if(this.wave.wave > 1){
                var bookName = $('#book').text()
                var haveReadBooks = localStorage.getItem('haveReadBooks')
                if(haveReadBooks){
                    try{
                       haveReadBooks = JSON.parse(haveReadBooks)
                       if(typeof haveReadBooks === 'object'){
                            if(haveReadBooks[num] && haveReadBooks[num].indexOf(bookName) === -1){
                                haveReadBooks[num].push(bookName)
                            } else if(!haveReadBooks[num]){
                                haveReadBooks[num] = [bookName]
                            }
                       }
                       
                    }catch(e){console.log(e)}
                    
                } else if(!haveReadBooks || haveReadBooks === 'null'){
                    haveReadBooks = {}
                    haveReadBooks[num] = [bookName]
                }
                try{
                   haveReadBooks = JSON.stringify(haveReadBooks)
                   localStorage.setItem('haveReadBooks', haveReadBooks)
                }catch(e){
                    console.log(e)
                }
                
            }
            $('#book').text(ig.bookList[wordIndex])
            $('#book').fadeIn()
            setTimeout(function(){
                $('#book').fadeOut()
            },2000)
        ...
        initCurrentTarget () {
            ...
            const nearestTargetList = []
            for (let i = 0; i < keys.length; i++) {
                const key = keys[i]
                if (this.targets[key].length) {
                    this.targets[key].forEach(item => {
                        nearestTargetList.push(item)
                    })
                }
            }
            var nearestDistance = -1;
            var nearestTarget = null;
            for (var i = 0; i < nearestTargetList.length; i++) {
                var distance = this.player.distanceTo(nearestTargetList[i]);
                if (distance < nearestDistance || !nearestTarget) {
                    nearestDistance = distance;
                    nearestTarget = nearestTargetList[i];
                }
            }
            if (nearestTarget) {
